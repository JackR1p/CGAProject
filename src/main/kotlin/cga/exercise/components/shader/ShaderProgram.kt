...
class ShaderProgram(vertexShaderPath: String, fragmentShaderPath: String) {
    ...
    private val vec2f: FloatBuffer = BufferUtils.createFloatBuffer(2)
    private val vec3f: FloatBuffer = BufferUtils.createFloatBuffer(3)
    ...
    fun setUniform(name: String, value: Vector3f): Boolean {
        ...
        if (loc != -1) {
            value.get(vec3f)
            GL20.glUniform3fv(loc, vec3f)
            return true
        }
        return false
    }

    fun setUniform(name: String, transposed: Boolean, value: Matrix4f): Boolean {
        if (programID == 0) return false
        val loc = GL20.glGetUniformLocation(programID, name)
        if (loc != -1) {
            value.get(m4x4buf)
            GL20.glUniformMatrix4fv(loc, transposed, m4x4buf)
            return true
        }
        return false
    }
